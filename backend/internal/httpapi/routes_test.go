...
func TestAdminPaymentSuccessIngestionLifecycle(t *testing.T) {
	...
	var jobCount int64
	if err := database.QueryRowContext(ctx, `SELECT COUNT(*) FROM fulfillment_jobs WHERE idempotency_key = ?;`, "idem-payment-ok").Scan(&jobCount); err != nil {
	...
	if err := database.QueryRowContext(ctx, `SELECT COUNT(*) FROM fulfillment_events WHERE fulfillment_job_id = ?;`, firstPayload.Job.ID).Scan(&eventCount); err != nil {
...
func TestUserGetFulfillmentJobOwnsAccess(t *testing.T) {
	...
	if otherUserID == ownerUserID {
		t.Fatalf("expected distinct users for ownership test")
	}
}
...
func TestAdminPaymentSuccessExecutesBalanceRechargeImmediately(t *testing.T) {
	...
	var eventCount int64
	if err := database.QueryRowContext(ctx, `SELECT COUNT(*) FROM fulfillment_events WHERE fulfillment_job_id = ?;`, payload.Job.ID).Scan(&eventCount); err != nil {
		t.Fatalf("count fulfillment events: %v", err)
	}
	if eventCount < 2 {
	...
	if err := database.QueryRowContext(ctx, `SELECT status FROM fulfillment_jobs WHERE id = ?;`, payload.Job.ID).Scan(&finalStatus); err != nil {
...
func TestAdminPaymentSuccessExecutesDelegatedAPIKeyCreationImmediately(t *testing.T) {
	...
	if _, err := database.ExecContext(ctx, `INSERT INTO sub2api_auth_tokens(user_id, access_token, refresh_token) VALUES (?, ?, ?);`, userID, "upstream-user-token", "upstream-refresh-token"); err != nil {
...
func TestAdminAvailableGroupsReturnsMappedGroups(t *testing.T) {
	...
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		...
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"code":0,"message":"ok","data":[{"id":42,"name":"Starter Group","code":"starter","platform":"openai","type":"subscription"},{"id":77,"name":"Balance Group","code":"wallet","platform":"openai","type":"balance"}]}`))
	}))
	t.Cleanup(upstream.Close)
	...
	if payload.Groups[0].ID != 42 || payload.Groups[0].Code != "starter" || payload.Groups[0].Type != "subscription" {
		t.Fatalf("unexpected first group payload: %+v", payload.Groups[0])
	}
	if payload.Groups[1].ID != 77 || payload.Groups[1].Code != "wallet" || payload.Groups[1].Type != "balance" {
...
func TestAdminReplayFulfillmentJobRequiresAdminAndRetriesRetryableJob(t *testing.T) {
	...
	database := setupTestDB(t)
	tierID := insertTier(t, ctx, database, "starter", "Starter")
	insertTierGroupBinding(t, ctx, database, tierID, "starter-group")
...
func TestAdminPackageLifecyclePersistsTierGroupBindings(t *testing.T) {
	...
	createReq := httptest.NewRequest(http.MethodPost, "/admin/packages", bytes.NewReader([]byte(`{"code":"starter","name":"Starter Package","group_codes":["starter-group","wallet-group"]}`)))
	setBearerAuth(createReq, adminSessionToken)
	createRec := httptest.NewRecorder()
	mux.ServeHTTP(createRec, createReq)
	...
	if len(created.GroupCodes) != 2 || created.GroupCodes[0] != "starter-group" || created.GroupCodes[1] != "wallet-group" {
		t.Fatalf("unexpected created groups: %+v", created.GroupCodes)
	}

	var bindingCount int64
	if err := database.QueryRowContext(ctx, `SELECT COUNT(*) FROM tier_group_bindings WHERE tier_id = (SELECT id FROM tiers WHERE code = ?);`, "starter").Scan(&bindingCount); err != nil {
	...
	if listPayload.Packages[0].Code != "starter" || len(listPayload.Packages[0].GroupCodes) != 2 {
	...
	updateReq := httptest.NewRequest(http.MethodPut, "/admin/packages/starter", bytes.NewReader([]byte(`{"name":"Starter Plus","group_codes":["wallet-group","pro-group"]}`)))
	...
	if len(updated.GroupCodes) != 2 || updated.GroupCodes[0] != "pro-group" || updated.GroupCodes[1] != "wallet-group" {
		t.Fatalf("unexpected updated groups: %+v", updated.GroupCodes)
	}

	rows, err := database.QueryContext(ctx, `SELECT group_code FROM tier_group_bindings WHERE tier_id = (SELECT id FROM tiers WHERE code = ?) ORDER BY group_code ASC;`, "starter")
	if err != nil {
	...
	for rows.Next() {
		...
		bindings = append(bindings, groupCode)
	}
	if err := rows.Err(); err != nil {
		t.Fatalf("iterate bindings: %v", err)
	}
	if len(bindings) != 2 || bindings[0] != "pro-group" || bindings[1] != "wallet-group" {
...
func TestAdminPackageValidationAndNotFound(t *testing.T) {
	...
	badCreateReq := httptest.NewRequest(http.MethodPost, "/admin/packages", bytes.NewReader([]byte(`{"name":"Missing Code","group_codes":["starter-group"]}`)))
	...
	duplicateReq := httptest.NewRequest(http.MethodPost, "/admin/packages", bytes.NewReader([]byte(`{"code":"existing","name":"Duplicate","group_codes":["starter-group"]}`)))
	...
	badUpdateReq := httptest.NewRequest(http.MethodPut, "/admin/packages/existing", bytes.NewReader([]byte(`{"name":"","group_codes":[]}`)))
...
func insertTierGroupBinding(t *testing.T, ctx context.Context, database *sql.DB, tierID int64, groupCode string) {
	t.Helper()

	_, err := database.ExecContext(ctx, `INSERT INTO tier_group_bindings(tier_id, group_code) VALUES (?, ?);`, tierID, groupCode)
	if err != nil {
...
func TestAdminPackageCRUDWithNewFields(t *testing.T) {
	...
	createBody, _ := json.Marshal(map[string]any{
		"code":          "pro-monthly",
		"name":          "Pro Monthly",
		"group_codes":   []string{"claude-basic", "gpt-4o"},
	...
	updateBody, _ := json.Marshal(map[string]any{
		...
		"group_codes":   []string{"claude-basic"},
