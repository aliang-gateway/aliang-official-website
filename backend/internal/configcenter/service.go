...
func (s *Service) CreateSoftwareConfig(ctx context.Context, cfg *model.SoftwareConfig) error {
	...
	result, err := s.db.ExecContext(ctx, `
		...
		VALUES (?, ?, ?, ?, ?, ?, ?)`,
		cfg.SoftwareCode, cfg.SoftwareName, cfg.GroupID, cfg.Description, boolToInt(cfg.IsEnabled), now, now,
	)
	if err != nil {
...
func (s *Service) GetSoftwareConfigByCode(ctx context.Context, code string) (*model.SoftwareConfig, error) {
	...
	).Scan(&cfg.ID, &cfg.SoftwareCode, &cfg.SoftwareName, &cfg.GroupID, &cfg.Description, &isEnabled, &cfg.CreatedAt, &cfg.UpdatedAt)
...
func (s *Service) ListSoftwareConfigs(ctx context.Context) ([]model.SoftwareConfig, error) {
	...
	for rows.Next() {
		var cfg model.SoftwareConfig
		var isEnabled int
		if err := rows.Scan(&cfg.ID, &cfg.SoftwareCode, &cfg.SoftwareName, &cfg.GroupID, &cfg.Description, &isEnabled, &cfg.CreatedAt, &cfg.UpdatedAt); err != nil {
...
func (s *Service) UpdateSoftwareConfig(ctx context.Context, code string, cfg *model.SoftwareConfig) error {
	...
	result, err := s.db.ExecContext(ctx, `
		...
		WHERE software_code = ?`,
		cfg.SoftwareName, cfg.GroupID, cfg.Description, boolToInt(cfg.IsEnabled), now, strings.TrimSpace(code),
	)
	if err != nil {
...
func (s *Service) DeleteSoftwareConfig(ctx context.Context, code string) error {
	result, err := s.db.ExecContext(ctx, `DELETE FROM als_software_configs WHERE software_code = ?`, strings.TrimSpace(code))
	if err != nil {
...
func (s *Service) RemoveTag(ctx context.Context, softwareConfigID int64, tag string) error {
	result, err := s.db.ExecContext(ctx, `DELETE FROM als_software_tags WHERE software_config_id = ? AND tag = ?`,
...
func (s *Service) CreateTemplate(ctx context.Context, tpl *model.ConfigTemplate) error {
	...
	result, err := s.db.ExecContext(ctx, `
		...
		VALUES (?, ?, ?, ?, ?, ?, ?)`,
		tpl.SoftwareConfigID, tpl.Name, tpl.Format, tpl.Content, boolToInt(tpl.IsDefault), now, now,
	)
	if err != nil {
...
func (s *Service) UpdateTemplate(ctx context.Context, templateID int64, tpl *model.ConfigTemplate) error {
	...
	result, err := s.db.ExecContext(ctx, `
		...
		WHERE id = ?`,
		tpl.Name, tpl.Format, tpl.Content, boolToInt(tpl.IsDefault), now, templateID,
	)
	if err != nil {
...
func (s *Service) DeleteTemplate(ctx context.Context, templateID int64) error {
	result, err := s.db.ExecContext(ctx, `DELETE FROM als_config_templates WHERE id = ?`, templateID)
	if err != nil {
...
func (s *Service) ListTemplates(ctx context.Context, softwareConfigID int64) ([]model.ConfigTemplate, error) {
	...
	for rows.Next() {
		...
		var isDefault int
		if err := rows.Scan(&tpl.ID, &tpl.SoftwareConfigID, &tpl.Name, &tpl.Format, &tpl.Content, &isDefault, &tpl.CreatedAt, &tpl.UpdatedAt); err != nil {
...
func (s *Service) GetDefaultTemplate(ctx context.Context, softwareConfigID int64) (*model.ConfigTemplate, error) {
	...
	).Scan(&tpl.ID, &tpl.SoftwareConfigID, &tpl.Name, &tpl.Format, &tpl.Content, &isDefault, &tpl.CreatedAt, &tpl.UpdatedAt)
	if errors.Is(err, sql.ErrNoRows) {
...
func (s *Service) DeleteGlobalVar(ctx context.Context, varKey string) error {
	result, err := s.db.ExecContext(ctx, `DELETE FROM als_global_template_vars WHERE var_key = ?`, strings.TrimSpace(varKey))
	if err != nil {
...
func (s *Service) GetDefaultConfigByTag(ctx context.Context, tag string, apiKey string) (*DefaultConfigResult, error) {
	...
	err := s.db.QueryRowContext(ctx, `
		...
		WHERE st.tag = ? AND sc.is_enabled = 1
		LIMIT 1`, tag,
	).Scan(&cfg.ID, &cfg.SoftwareCode, &cfg.SoftwareName, &cfg.GroupID, &cfg.Description, &isEnabled, &cfg.CreatedAt, &cfg.UpdatedAt)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, ErrNotFound
...
func (s *Service) SyncConfigs(ctx context.Context, userID int64, configs []model.UserSyncedConfig) error {
	...
	for _, c := range configs {
		...
		_, err := s.db.ExecContext(ctx, `
			...
			boolToInt(c.InUse), boolToInt(c.Selected), c.Format, c.Content, now, now,
			c.Software, c.Name, c.FilePath, c.Version,
			boolToInt(c.InUse), boolToInt(c.Selected), c.Format, c.Content, now,
		)
		if err != nil {
...
func (s *Service) PullConfigs(ctx context.Context, userID int64, opts PullConfigsOptions) ([]model.UserSyncedConfig, error) {
	...
	for rows.Next() {
		...
		var inUse, selected int
		if err := rows.Scan(&c.ID, &c.UserID, &c.UUID, &c.Software, &c.Name, &c.FilePath, &c.Version, &inUse, &selected, &c.Format, &c.Content, &c.CreatedAt, &c.UpdatedAt); err != nil {
...
func (s *Service) DeleteSyncedConfig(ctx context.Context, userID int64, uuid string) error {
	result, err := s.db.ExecContext(ctx, `DELETE FROM als_user_synced_configs WHERE user_id = ? AND uuid = ?`, userID, uuid)
	if err != nil {
